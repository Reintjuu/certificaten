...
export function drawFitnessChart(
  ...
  const yOf = (value: number) => bottom - ((value - min) / span) * plotHeight

  const plotLine = (pick: (record: GenerationRecord) => number, color: string) => {
...
function drawLegend(ctx: CanvasRenderingContext2D, height: number, maxSolved: number) {
